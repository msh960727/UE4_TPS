...
ACBullet_ChangeMaterial::ACBullet_ChangeMaterial()
{
...
void ACBullet_ChangeMaterial::BeginPlay()
{
	Super::BeginPlay();
	...
	for (USphereComponent* sphere : components)
		sphere->OnComponentBeginOverlap.AddDynamic(this, &ACBullet_ChangeMaterial::OnComponentBeginOverlap);
}

void ACBullet_ChangeMaterial::OnComponentBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	//if (!!Explosion)
//{
...
//}
	...
	if (OnChangeBulletBeginOverlap.IsBound())
		OnChangeBulletBeginOverlap.Broadcast(SweepResult);
